...
int main(int argc, char** argv) {
  ...
  string buf;
	...
	if(discourse_cnt>0){
        ...
        //which disc variations to enable
        std::vector<bool> b;
        bool isdisc0 = decoder.GetConf().count("discourse_0");
		bool isdisc1 = decoder.GetConf().count("discourse_1");
		bool isdisc2 = decoder.GetConf().count("discourse_2");
        b.push_back(isdisc0);
        b.push_back(isdisc1);
        b.push_back(isdisc2);
        ff_discourse->set_isdisc(b);
  ...
  int cur_pass = conf["passes"].as<int>();
  ...
  // while (cur_pass <= max_iteration) {
    ...
    while(*in) {
      ...
      decoder.Decode(buf, &observer);  // update oracles
		...
		if(optimize_again)
		  {
			...
			if(cur_pair.empty()){iter=1000; cerr << "Undefined pair " << endl; continue;} //pair is undefined so we are done with this smo 
