...
std::string viterbi_stats(Hypergraph const& hg, std::string const& name, bool estring, bool etree,bool show_derivation, bool show_rules, std::string rules_file)
{
  ostringstream o;
  o << hg.stats(name);
  if (estring) {
  ...
  if (show_rules) {
    o<<name<<"         rules: "<<endl;
    hg.print_viterbi_ruleset(rules_file);
  }
