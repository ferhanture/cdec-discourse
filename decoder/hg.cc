...
void Hypergraph::PrintRules(std::string file) {
	int ei = 0;
	
	WriteFile out_write(file);
	ostream& out = out_write.get();
	...
	for (vector<Edge>::const_iterator i = edges_.begin();
		...
		string rule = (edge.rule_ ? edge.rule_->AsString() : none);
		out << "1.0 " << rule << std::endl << flush;
		//	  std::cerr << rule << " /prob = " << edge.edge_prob_ << std::endl << flush;
	}
	std::cerr << "wrote " << ei << " rules to " << file << std::endl; 
}
...
void Hypergraph::print_viterbi_ruleset(std::string rules_file) const {
	HypergraphP v=CreateViterbiHypergraph();
	v->PrintRules(rules_file);
}
